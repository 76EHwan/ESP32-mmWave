...
//
//  con/cmd 는 실제 하드웨어(G60SM1SY / R60A) 로그로 확인한 값이다.
//  이 모듈은 요청 없이 능동 보고를 하며, 보고는 0x0X, 질의 응답은 0x8X 를 쓴다.
//  다만 con=0x84(수면) 는 0x0C(종합 8바이트)와 0x8C(보고 모드 1바이트)처럼
//  두 계열이 충돌하므로, 상위 비트를 떼지 않고 (con,cmd) 쌍을 그대로 매핑한다.
// ============================================================================

class C1001Passive {
public:
    static const uint16_t MAX_DATA = 64;   // 이보다 긴 프레임은 버린다
    ...
    /// 파서 상태와 통계를 초기화한다. 포트의 begin() 은 호출자가 미리 한다.
    /// 모듈은 리셋 직후 전체 상태를 한꺼번에 쏟아내므로, 그 덤프가 끝난 뒤
    /// 다시 한 번 불러 통계를 리셋해야 갱신 주기가 제대로 나온다
    void begin();
    ...
    uint32_t oversize = 0;      ///< len 이 MAX_DATA 를 넘어 버린 프레임
    uint32_t lastFrameMs = 0;   ///< 마지막 프레임 수신 시각
    uint32_t startMs = 0;       ///< begin() 시각
...
private:
    ...
    static const uint8_t WAVE_N = 32;
    uint8_t _respRing[WAVE_N], _heartRing[WAVE_N];
    ...
    uint16_t _sum = 0;
    uint8_t  _data[MAX_DATA];

    // 모르는 프레임을 한 번씩만 찍기 위한 목록.
    // 이 모듈은 부팅 시 30종 가까이 쏟아내므로 넉넉히 잡는다
    static const uint8_t UNK_SLOTS = 64;
    uint16_t _unkSeen[UNK_SLOTS];
    uint8_t  _unkCount = 0;
