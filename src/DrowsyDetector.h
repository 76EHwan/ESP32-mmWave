...
// --- 체동 ---
#define MOVE_TAU_S          15.0f   // 체동 이동평균 시정수 [초]
...
#define MOVE_QUIET_SPIKE    30      // 이 값을 넘으면 "방금 움직였다"
#define STILL_AFTER_SPIKE_MS 5000UL // 스파이크 후 이만큼 지나야 다시 정지로 본다

// 스파이크 없이 잔움직임만 계속되는 경우를 걸러내는 느슨한 상한.
// 눈 감고 앉은 구간의 최대가 11.6 이었으므로 그 위로 잡는다
#define MOVE_QUIET_MAX      15.0f
...
#define MOVE_SPIKE          40

// --- 심박 ---
#define HR_MIN              40      // 유효 심박 하한
#define HR_MAX              150     // 유효 심박 상한
#define HR_BLANK_MS         60000UL // 체동 이벤트 후 심박을 버릴 시간.
                                    // 추정기 수렴에 1~2분 걸리지만, 90초로 두면
                                    // 스파이크가 한 번만 나도 창이 통째로 비어
                                    // 표본이 안 쌓인다. 60초로 타협했다
#define HR_WIN_MS           120000UL// 중앙값을 낼 창 [ms]
#define HR_WIN_N            64      // 링버퍼 크기 (3초 주기면 120초에 40샘플)
#define HR_WIN_MIN_N        8       // 중앙값을 신뢰할 최소 샘플 수
...
#define HR_BASE_MIN_N       20
#define HR_BASE_TAU_S       600.0f  // 각성 기준선 시정수 [초]
#define HR_DROP_BPM         4.0f    // 기준선 대비 이만큼 떨어지면 졸음 징후
...
#define HR_DOWN_HOLD_MS     (30000UL * HOLD_SCALE_PCT / 100)
...
#define LOCK_DIST_MIN       30
#define LOCK_DIST_MAX       170

// 거리는 2초마다 갱신되는데 가끔 한 샘플만 엉뚱하게 튄다. 그 한 번에
// 락온이 끊기지 않도록, 연속으로 이만큼 벗어나야 상태를 바꾼다
#define LOCK_DEBOUNCE_N     3
...
#define HOLD_NO_EVIDENCE_MS (180000UL * HOLD_SCALE_PCT / 100)  // 체동만 (3분)
#define HOLD_ONE_MS         (60000UL  * HOLD_SCALE_PCT / 100)  // 증거 1개 (1분)
#define HOLD_TWO_MS         (25000UL  * HOLD_SCALE_PCT / 100)  // 증거 2개 (25초)
#define AWAKE_HOLD_MS       (10000UL  * HOLD_SCALE_PCT / 100)  // 각성 복귀

// 이 값을 넘는 체동은 자리에서 일어나는 수준이다. 각성 복귀를 기다리지 않고
// 즉시 DROWSY 를 해제한다. 실측에서 일어설 때 84, 100, 57 이 찍혔는데
// 10초 동안 DROWSY 가 남아 있었다
#define WAKE_SPIKE          40
...
public:
    // NOLOCK : 사람은 감지되는데 그 사람을 제대로 못 물고 있는 상태.
    ...
    enum State : uint8_t { NOPERSON, NOLOCK, WARMUP, AWAKE, DROWSY };
    ...
    uint32_t    hrDownMs(uint32_t now) const {
        return _hrDownArmed ? (now - _hrDownSince) : 0;
    }
    uint8_t     evidence() const { return (uint8_t)(_respLost + _hrDown); }
    ...
    /// MOVE_SPIKE 가 그 환경에 비해 너무 낮아 심박이 영영 안 쌓인다
    uint8_t     blankPercent() const {
        return _ticks ? (uint8_t)(100UL * _blankTicks / _ticks) : 0;
    }

private:
    void pushHr(uint8_t v, uint32_t t);

    State    _state = NOPERSON;
    uint32_t _startMs = 0;
    ...
    bool     _locked = false;               // bed==1, 모듈이 사람을 물고 있는가

    uint16_t _presence = 0, _bed = 0, _dist = 0;
    uint8_t  _distIn = 0, _distOut = 0;     // 거리 범위 연속 진입/이탈 횟수
    uint16_t _respRate = 0, _respState = RESP_STATE_NONE;

    float    _moveAvg = 0.0f;
    uint32_t _lastMoveMs = 0;
    uint32_t _lastSpikeMs = 0;      // 마지막으로 MOVE_QUIET_SPIKE 를 넘은 시각
    uint32_t _hrBlankUntil = 0;

    uint8_t  _hrBuf[HR_WIN_N];
    uint32_t _hrTime[HR_WIN_N];
