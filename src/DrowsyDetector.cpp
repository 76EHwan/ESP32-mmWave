...
void DrowsyDetector::begin(uint32_t now)
{
    _state = NOPERSON;
    _startMs = now;
    _cPresence = _cBed = _cBody = _cResp = _cHr = 0;
    _presence = _bed = _dist = 0;
    _distIn = _distOut = 0;
    _locked = false;
    _respRate = 0;
    _respState = RESP_STATE_NONE;
    _moveAvg = 0.0f;
    _lastMoveMs = now;
    _lastSpikeMs = now;
    _hrBlankUntil = now;
...
void DrowsyDetector::update(const C1001Passive &r, uint32_t now)
{
    ...
    if (!_seeded) {
        ...
        if (r.respRate.valid)  { _cResp = r.respRate.count;      onRespiration(r.respRate.value, r.respState.value); }
...
// 연속 LOCK_DEBOUNCE_N 번 같은 방향이어야 _locked 를 바꾼다
void DrowsyDetector::onDistance(uint16_t cm)
{
    _dist = cm;
    if (cm >= LOCK_DIST_MIN && cm <= LOCK_DIST_MAX) {
        _distOut = 0;
        if (_distIn < LOCK_DEBOUNCE_N) _distIn++;
        if (_distIn >= LOCK_DEBOUNCE_N) _locked = true;
    } else {
        _distIn = 0;
        if (_distOut < LOCK_DEBOUNCE_N) _distOut++;
        if (_distOut >= LOCK_DEBOUNCE_N) _locked = false;
    }
}
...
void DrowsyDetector::onBodyMove(uint16_t v, uint32_t t)
{
    ...
    _moveAvg += (dt / (MOVE_TAU_S + dt)) * ((float)v - _moveAvg);

    // 정지 판정의 기준. 타이핑·마우스·자세 고치기가 여기에 걸린다
    if (v > MOVE_QUIET_SPIKE) {
        _lastSpikeMs = t;
    }
...
void DrowsyDetector::pushHr(uint8_t v, uint32_t t)
{
    _hrBuf[_hrHead] = v;
    _hrTime[_hrHead] = t;
    _hrHead = (uint8_t)((_hrHead + 1) % HR_WIN_N);
    if (_hrFill < HR_WIN_N) _hrFill++;
}

void DrowsyDetector::onHeartRate(uint16_t bpm, uint32_t t)
{
    bool ok = (bpm >= HR_MIN) && (bpm <= HR_MAX);
    ...
    if (_hrBase == 0.0f) {
        ...
        if (hrSamples(t) < HR_BASE_MIN_N) return;
        _hrBase = med;
    } else if (!_drowsy) {
        if (dt <= 0.0f) dt = 0.001f;
        _hrBase += (dt / (HR_BASE_TAU_S + dt)) * ((float)med - _hrBase);
    }
}

bool DrowsyDetector::hrBlanking(uint32_t now) const
{
    return (int32_t)(_hrBlankUntil - now) > 0;
}
...
uint8_t DrowsyDetector::hrSamples(uint32_t now) const
{
    ...
    for (uint8_t i = 0; i < _hrFill; i++) {
        if ((now - _hrTime[i]) <= HR_WIN_MS) n++;
    }
    return n;
}

// 최근 HR_WIN_MS 안의 샘플로 중앙값을 낸다. 표본이 부족하면 0
uint8_t DrowsyDetector::hrMedian(uint32_t now) const
{
    ...
    for (uint8_t i = 0; i < _hrFill; i++) {
        if ((now - _hrTime[i]) <= HR_WIN_MS) tmp[n++] = _hrBuf[i];
    }
...
{
    uint8_t med = hrMedian(now);
    if (_hrBase <= 0.0f || med == 0) return 0.0f;
    return _hrBase - (float)med;
}

uint32_t DrowsyDetector::needMs() const
{
    uint8_t ev = evidence();
    return (ev >= 2) ? HOLD_TWO_MS : (ev == 1) ? HOLD_ONE_MS : HOLD_NO_EVIDENCE_MS;
}
...
void DrowsyDetector::tick(uint32_t now)
{
    ...
    _respLost = present && _locked &&
                (_respRate == 0 || _respState == RESP_STATE_NONE);

    // 보조 증거 (2) 심박 중앙값이 기준선보다 유의하게 낮은 상태가 지속된다.
    // 순간적으로 넘는 것은 각성 중에도 일어나므로 지속 시간을 함께 요구한다
    bool hrDownRaw = (hrMedian(now) > 0) && (hrDrop(now) >= HR_DROP_BPM);
    if (hrDownRaw) {
        if (!_hrDownArmed) {
            _hrDownArmed = true;
            _hrDownSince = now;
    ...
    _hrDown = _hrDownArmed && ((now - _hrDownSince) >= HR_DOWN_HOLD_MS);
    ...
    bool still = ((now - _lastSpikeMs) >= STILL_AFTER_SPIKE_MS) &&
                 (_moveAvg < MOVE_QUIET_MAX);
    bool sign  = present && _locked && still;
    ...
    uint32_t held = now - _signSince;
    if (!_drowsy && sign && held >= needMs()) {
        _drowsy = true;
    } else if (_drowsy && !sign && held >= AWAKE_HOLD_MS) {
        _drowsy = false;
    }
    ...
    if (!present) {
        ...
        _state = NOLOCK;    // 가림/거리 이탈. 졸음 여부를 알 수 없다
    } else if (_drowsy) {
        _state = DROWSY;
    } else if (!_hrEverValid && (now - _startMs) < WARMUP_MS) {
        _state = WARMUP;    // 아직 호흡·심박이 한 번도 안 잡혔다
    } else {
        _state = AWAKE;
    }
}

const char *DrowsyDetector::stateName() const
{
    switch (_state) {
    case NOPERSON: return "NOPERSON";
